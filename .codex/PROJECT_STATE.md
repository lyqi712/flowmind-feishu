...
- Real Feishu credentials were configured by the user (App ID redacted) through the encrypted secret store and a real sync ran against the default data directory (server on 127.0.0.1:8789).
...
- DeepSeek configured and verified live (2026-08-12 20:10+): user-provided key saved encrypted; base is a user-configured OpenAI-compatible endpoint, model `deepseek-v4-flash` (deepseek-chat unsupported by account); real streaming chat + real-model summary Skill verified with citations (`fallbackUsed: false`).
...
## 2026-09-09 1.3.1 公开发布
- 本地优化范围：后端启动语法、知识依赖代码请求分流、回答改写来源继承、明确写入确认、SQLite canonical notes、备份恢复事务、笔记版本冲突/顶层导航保存保护、导入身份/失败语义/原件/事务、统一公网网页读取安全边界、搜索分类、Electron main ESM启动路径。
- 上述改动纳入 FlowMind 1.3.1 源码与 Windows 安装包/便携版。公开仓库不包含 `evidence/`、本机 `runtime-data/`、IMA 逆向材料和安装器二进制。
