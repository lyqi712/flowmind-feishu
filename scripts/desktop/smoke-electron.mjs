...
const userDataDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'flowmind-electron-smoke-'));
await fs.promises.rm(resultFile, { force: true });

const child = spawn(electronExe, [path.join(appRoot, 'desktop'), '--disable-gpu', `--user-data-dir=${userDataDir}`], {
  cwd: appRoot,
  windowsHide: true,
