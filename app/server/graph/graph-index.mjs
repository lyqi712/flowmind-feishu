...
function isFeishuUrl(value) {
  return /^https?:\/\/(?:[^/]+\.)?(?:feishu|larksuite)\.(?:cn|com)\//iu.test(String(value || '').trim());
}

function parseMetadataTarget(value) {
  const raw = clean(value);
  if (!raw) return { target: '', anchor: null };
  const hash = !/^https?:\/\//iu.test(raw) ? raw.indexOf('#') : -1;
  return {
    target: clean(hash >= 0 ? raw.slice(0, hash) : raw),
    anchor: clean(hash >= 0 ? raw.slice(hash + 1) : '') || null
  };
}

function metadataLinkValues(value) {
...
function linkValue(entry, names) {
  for (const name of names) {
    const value = entry?.[name];
    if (typeof value === 'string' && clean(value)) return value;
    if (isRecord(value)) {
      const nested = linkValue(value, ['documentId', 'contentItemId', 'externalId', 'target', 'url', 'href', 'link', 'id']);
...
export function parseMetadataLinks(value) {
  const links = [];
  for (const [index, entry] of metadataLinkValues(value).entries()) {
    const record = isRecord(entry) ? entry : { target: entry };
    const parsed = parseMetadataTarget(linkValue(record, ['documentId', 'contentItemId', 'externalId', 'target', 'url', 'href', 'link', 'id']));
    if (!parsed.target || isPlaceholderTarget(parsed.target)) continue;
    if (EXTERNAL_TARGET.test(parsed.target) && !isFeishuUrl(parsed.target)) continue;
    const sourceAnchor = clean(record.sourceAnchor || record.blockAnchor || record.fromAnchor || record.location?.anchor || '') || `metadata:links:${index + 1}`;
    const targetAnchor = clean(record.targetAnchor || record.toAnchor || record.destinationAnchor || record.anchor || parsed.anchor || '') || null;
    links.push({
      kind: 'metadata-link',
      target: parsed.target,
      normalizedTarget: normalizeLinkTarget(parsed.target),
      sourceAnchor,
      targetAnchor,
      label: clean(record.label || record.title || record.text || record.name || '') || null,
      raw: typeof entry === 'string' ? entry : structuredClone(record)
    });
  }
  return links;
}
...
function nodeFromRow(row) {
  ...
  return {
    ...
    contentVersionId: row.version_id ?? null,
...
function sourceRefIsExplicit(ref, validateAgentEvidence = null) {
  const provenance = ref?.provenance && typeof ref.provenance === 'object' ? ref.provenance : {};
  const kind = clean(provenance.kind || ref?.kind || ref?.relationKind || '').toLocaleLowerCase();
  if (kind === 'agent-evidence') {
    return Boolean(clean(provenance.evidenceId) && clean(provenance.signature) && clean(ref?.documentId || ref?.contentItemId) && typeof validateAgentEvidence === 'function' && validateAgentEvidence(ref) === true);
  }
  if (kind) return ['manual-source', 'source-reference', 'explicit-source', 'explicit', 'user-source'].includes(kind);
  // Older user-authored notes did not persist provenance. Their source references are explicit by author intent.
  return Boolean(clean(ref?.documentId || ref?.contentItemId));
}
...
export class GraphIndex {
  constructor({ repository, clock = () => new Date(), validateAgentEvidence = null } = {}) {
    if (!repository?.db) throw new TypeError('repository with a SQLite database is required');
    this.repository = repository;
    this.db = repository.db;
    this.clock = clock;
    this.validateAgentEvidence = typeof validateAgentEvidence === 'function' ? validateAgentEvidence : null;
  }
  ...
  rebuild() {
    ...
    const sourceTypeById = new Map(this.repository.listSourceConnections({ includeDeleted: false }).map(source => [source.id, source.sourceType]));
    ...
    return this.repository.transaction(() => {
      ...
      // Production FKs are ON DELETE SET NULL. Snapshot approved suggestions
      // before DELETE, or rebuild would wipe the node ids and the replay would no-op.
      const replayableSuggestions = this.snapshotReplayableSuggestions();
      ...
      for (const { item, node } of itemsById.values()) {
        for (const value of [item.id, item.externalId, item.title, node.path, item.sourceUrl, item.metadata?.url, item.metadata?.legacyId, ...(node.aliases || [])]) addLookup(value, node.id);
      }
      ...
      for (const { item, node, parsed } of itemsById.values()) {
        ...
        for (const link of parseMetadataLinks(item.metadata?.links)) {
          const resolved = resolveTarget(link.target);
          const targetNode = resolved.nodeId ? nodes.get(resolved.nodeId) : null;
          addEdge({
            from: node.id,
            to: resolved.nodeId,
            type: 'link',
            ...
            createdSource: sourceTypeById.get(item.sourceConnectionId) === 'feishu' ? 'feishu-metadata-link' : 'content-metadata-link',
            sourceItemId: item.id,
            sourceVersionId: node.versionId,
            rawTarget: link.target,
            provenance: {
              kind: 'explicit-link',
              parser: 'metadata.links',
              raw: link.raw,
              sourceAnchor: link.sourceAnchor,
              targetAnchor: link.targetAnchor,
              contentHash: node.contentHash,
              targetVersionId: targetNode?.versionId || null
            }
          });
        }
        ...
        if (node.type === 'note') {
          for (const ref of safeArray(item.metadata?.sourceRefs)) {
            if (!sourceRefIsExplicit(ref, this.validateAgentEvidence)) continue;
            const targetId = clean(ref?.documentId || ref?.contentItemId || ref?.id);
            const resolved = resolveTarget(targetId || ref?.title || '');
            if (resolved.status !== 'resolved') continue;
            const targetItem = targetId ? itemsById.get(targetId)?.item || this.repository.getContentItem(targetId, { includeDeleted: true, includeTags: false }) : null;
            const boundEvidence = bindEvidenceRef({
              documentId: targetId || null,
              title: ref?.title || targetItem?.title,
              anchor: ref?.anchor || null,
              excerpt: ref?.excerpt || ref?.quote || ref?.snippet || '',
            ...
            const boundSource = ref?.evidenceStatus === 'unverified' && boundEvidence.evidenceStatus === 'current'
              ? { ...boundEvidence, evidenceStatus: 'unverified', evidenceStatusReason: ref.evidenceStatusReason || 'source_location_not_observed' }
              : boundEvidence;
            addEdge({
              from: node.id,
              to: resolved.nodeId,
              ...
              provenance: { kind: 'source-reference', sourceRef: boundSource }
            });
          }
        }
      }
      for (const suggestion of replayableSuggestions) {
        const restored = this.restoreSuggestionIdentity(suggestion);
        if (restored.status === 'approved') this.insertApprovedSuggestionEdge(restored);
      }
  ...
  snapshot({ spaceId = '', includeSuggestions = false } = {}) {
    ...
    const projectedNodes = nodes.map(node => {
      const item = node.sourceId ? this.repository.getContentItem(node.sourceId, { includeDeleted: true, includeTags: false }) : null;
      const sourceRef = item && node.type !== 'tag' && node.type !== 'folder'
        ? bindEvidenceRef({ documentId: item.id, title: item.title, contentVersionId: node.versionId, contentHash: node.contentHash, excerpt: '' }, item)
        : null;
      return { ...node, ...(sourceRef ? { sourceRef, evidenceStatus: sourceRef.evidenceStatus, evidenceStatusReason: sourceRef.evidenceStatusReason } : {}), degree: degree.get(node.id) || 0, orphan: node.type !== 'tag' && node.type !== 'folder' && (degree.get(node.id) || 0) === 0 };
    });
  ...
  hasResolvedPair(sourceNodeId, targetNodeId, edgeType = 'link') {
    if (!sourceNodeId || !targetNodeId) return false;
    const row = this.db.prepare(`SELECT id FROM graph_edges
      WHERE deleted_at IS NULL AND parsing_status = 'resolved' AND edge_type = ?
        AND ((source_node_id = ? AND target_node_id = ?) OR (source_node_id = ? AND target_node_id = ?))
      LIMIT 1`).get(edgeType, sourceNodeId, targetNodeId, targetNodeId, sourceNodeId);
    return Boolean(row);
  }

  findOpenSuggestionPair(sourceNodeId, targetNodeId, edgeType = 'link') {
    if (!sourceNodeId || !targetNodeId) return null;
    return this.listSuggestions().find(suggestion => {
      if (!['pending', 'approved'].includes(suggestion.status)) return false;
      if ((suggestion.edgeType || 'link') !== edgeType) return false;
      return (suggestion.sourceNodeId === sourceNodeId && suggestion.targetNodeId === targetNodeId)
        || (suggestion.sourceNodeId === targetNodeId && suggestion.targetNodeId === sourceNodeId);
    }) || null;
  }
  ...
  createSuggestion({ sourceNodeId, targetNodeId, edgeType = 'link', reason = '', evidence = [], proposedContentItemId = null, proposedPatch = {}, createdSource = 'agent' } = {}) {
    ...
    const sourceItem = this.db.prepare('SELECT content_item_id FROM graph_nodes WHERE id = ?').get(sourceNodeId);
    const targetItem = this.db.prepare('SELECT content_item_id FROM graph_nodes WHERE id = ?').get(targetNodeId);
    const patch = {
      ...(isRecord(proposedPatch) ? proposedPatch : {}),
      sourceContentItemId: sourceItem?.content_item_id || proposedPatch?.sourceContentItemId || null,
      targetContentItemId: targetItem?.content_item_id || proposedPatch?.targetContentItemId || null
    };
    this.db.prepare(`INSERT INTO graph_suggestions(
      ...
      stringify(patch), createdSource, timestamp, timestamp);
  ...
  snapshotReplayableSuggestions() {
    return this.listSuggestions().filter(suggestion => suggestion.status === 'approved' || suggestion.status === 'pending').map(suggestion => {
      const source = suggestion.sourceNodeId
        ? this.db.prepare('SELECT id, content_item_id FROM graph_nodes WHERE id = ?').get(suggestion.sourceNodeId)
        : null;
      const target = suggestion.targetNodeId
        ? this.db.prepare('SELECT id, content_item_id FROM graph_nodes WHERE id = ?').get(suggestion.targetNodeId)
        : null;
      const patch = isRecord(suggestion.proposedPatch) ? suggestion.proposedPatch : {};
      return {
        ...suggestion,
        sourceContentItemId: source?.content_item_id || patch.sourceContentItemId || null,
        targetContentItemId: target?.content_item_id || patch.targetContentItemId || null
      };
    });
  }

  restoreSuggestionIdentity(suggestion) {
    const resolve = (nodeId, contentItemId) => {
      if (nodeId && this.db.prepare('SELECT id FROM graph_nodes WHERE id = ? AND deleted_at IS NULL').get(nodeId)) return nodeId;
      if (contentItemId) {
        const node = this.getNodeByContentItem(contentItemId);
        if (node?.id) return node.id;
      }
      return null;
    };
    const sourceNodeId = resolve(suggestion.sourceNodeId, suggestion.sourceContentItemId);
    const targetNodeId = resolve(suggestion.targetNodeId, suggestion.targetContentItemId);
    if (sourceNodeId && targetNodeId) {
      this.db.prepare('UPDATE graph_suggestions SET source_node_id = ?, target_node_id = ? WHERE id = ?')
        .run(sourceNodeId, targetNodeId, suggestion.id);
    }
    return { ...suggestion, sourceNodeId, targetNodeId };
  }
  ...
  insertApprovedSuggestionEdge(suggestion) {
    if (!suggestion || suggestion.status !== 'approved') return false;
    const source = this.db.prepare('SELECT * FROM graph_nodes WHERE id = ? AND deleted_at IS NULL').get(suggestion.sourceNodeId);
    const target = this.db.prepare('SELECT * FROM graph_nodes WHERE id = ? AND deleted_at IS NULL').get(suggestion.targetNodeId);
    if (!source || !target) return false;
    const timestamp = now(this.clock);
    const edgeType = suggestion.edgeType || 'link';
    const label = clean(suggestion.reason) || '已确认建议';
    const edgeId = stableId('edge', suggestion.sourceNodeId, suggestion.targetNodeId, edgeType, '', '', '', suggestion.id);
    const existing = this.db.prepare('SELECT id FROM graph_edges WHERE id = ? AND deleted_at IS NULL').get(edgeId);
    if (existing) return true;
    this.db.prepare(`INSERT INTO graph_edges(
      id, source_node_id, target_node_id, edge_type, directed, source_anchor, target_anchor,
      label, parsing_status, created_source, source_content_item_id, source_version_id,
      raw_target, occurrence, provenance_json, created_at, updated_at, deleted_at
    ) VALUES(?, ?, ?, ?, 1, NULL, NULL, ?, 'resolved', 'suggestion-approved', ?, ?, ?, 1, ?, ?, ?, NULL)`).run(
      edgeId,
      suggestion.sourceNodeId,
      suggestion.targetNodeId,
      edgeType,
      label,
      source.content_item_id || null,
      source.version_id || null,
      target.content_item_id || target.title || suggestion.targetNodeId,
      stringify({ kind: 'approved-suggestion', suggestionId: suggestion.id, reason: suggestion.reason || '' }),
      timestamp,
      timestamp
    );
    return true;
