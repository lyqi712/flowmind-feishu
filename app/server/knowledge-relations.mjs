...
import { appendWikiLinksToNote, applyAssistantAnswerToProblemNote, problemNoteDraft } from '../src/workspace/note-capture.js';
...
function normalizeChunk(chunk, index) {
  ...
  return {
    ...
    contentVersionId: chunk?.contentVersionId ?? metadata.contentVersionId ?? null,
    revision: normalizeSpace(chunk?.revision ?? metadata.revision ?? '') || null,
    contentHash: normalizeSpace(chunk?.sourceContentHash ?? metadata.sourceContentHash ?? '') || null
  };
}
...
function normalizeDocument(document, index, chunksByDocument) {
  ...
  return {
    ...
    content,
    chunks: chunks.length ? chunks : [{ id: 'document-content', text: content, anchor: null, pageNumber: null, region: null, timeStart: null, timeEnd: null, speaker: null, contentVersionId: document?.currentVersionId ?? null, revision: document?.revision || null, contentHash: document?.contentHash || null }]
  };
}
...
function makeSourceRef(document, chunk = {}, excerpt = '') {
  const source = {
    ...
    contentHash: chunk.sourceContentHash ?? chunk.contentHash ?? document.contentHash ?? null
  };
  ...
  return bindEvidenceRef(source, document, { excerpt: source.excerpt, anchor: source.anchor });
}
...
function normalizeCitation(citation, documentsById) {
  ...
  const document = documentsById.get(documentId) || {
    ...
    currentVersionId: citation?.currentVersionId ?? null,
    revision: citation?.currentRevision ?? null,
    contentHash: citation?.currentContentHash ?? null
  };
  const bound = bindEvidenceRef({
    documentId,
    title: document.title,
    evidenceId: citation?.evidenceId,
    evidenceSchemaVersion: citation?.evidenceSchemaVersion,
    ...
    contentVersionId: citation?.contentVersionId ?? citation?.sourceVersionId,
    revision: citation?.revision ?? citation?.sourceRevision,
    contentHash: citation?.contentHash ?? citation?.sourceContentHash
  }, document, { excerpt: citation?.excerpt || citation?.content || '', anchor: citation?.anchor, chunkId: citation?.chunkId, sourceId: citation?.sourceId });
  return citation?.evidenceStatus === 'unverified' && bound.evidenceStatus === 'current'
    ? { ...bound, evidenceStatus: 'unverified', evidenceStatusReason: citation.evidenceStatusReason || 'source_location_not_observed' }
    : bound;
}

function sourceRefKey(source) {
  return [source.documentId, source.anchor || '', source.excerpt || '', source.contentVersionId ?? '', source.revision || '', source.contentHash || ''].join('|');
}
...
export function candidateRelationSuggestionsFromRelations(relations = {}, { citations = [] } = {}) {
  const cited = new Set(citedDocumentIds(citations));
  if (cited.size < 2) return [];

  const pairs = [];
  const seen = new Set();

  function addPair(fromId, toId, reason, evidence, strength = 0) {
    const sourceContentItemId = normalizeSpace(fromId);
    const targetContentItemId = normalizeSpace(toId);
    if (!sourceContentItemId || !targetContentItemId || sourceContentItemId === targetContentItemId) return;
    if (!cited.has(sourceContentItemId) || !cited.has(targetContentItemId)) return;
    const key = [sourceContentItemId, targetContentItemId].sort().join('');
    ...
    pairs.push({
      ...
      targetContentItemId,
      edgeType: 'link',
      reason: sanitizeText(reason || '回答同时引用了这两篇资料', 240),
      evidence: Array.isArray(evidence) ? evidence.slice(0, 4) : [],
      strength: Number(strength) || 0
    });
  }

  for (const link of relations?.knowledgeMap?.bidirectionalLinks || []) {
    addPair(
      link.fromDocumentId,
      link.toDocumentId,
      link.reason,
      [
        { documentId: link.fromDocumentId, title: link.fromTitle, excerpt: link.reason },
        { documentId: link.toDocumentId, title: link.toTitle, excerpt: link.reason }
      ],
      link.strength
    );
  }
  ...
  for (const conflict of relations?.conflicts || []) {
    const views = Array.isArray(conflict.viewpoints) ? conflict.viewpoints : [];
    if (views.length >= 2) {
      addPair(
        views[0].documentId,
        views[1].documentId,
        conflict.explanation || conflict.topic || '两篇资料对同一议题存在分歧',
        conflict.sourceRefs || [],
        80
      );
    }
...
function artifactTags(kind, relations) {
  ...
  const kindTag = kind === 'note' ? '\u77e5\u8bc6\u7b14\u8bb0' : kind === 'task' ? '\u884c\u52a8\u4efb\u52a1' : kind === 'writing' ? '\u5199\u4f5c\u8349\u7a3f' : kind === 'problem' ? '\u95ee\u9898\u8bb0\u5f55' : '\u8bc1\u636e\u56fe\u8868';
...
function artifactSources(citations, relations) {
  ...
  for (const relation of relations?.relatedDocuments || []) {
    const documentId = normalizeSpace(relation.documentId);
    const source = (relation.sourceRefs || []).find(ref => ref?.currentVersionId != null || ref?.contentVersionId != null || ref?.revision || ref?.contentHash) || {};
    if (!documentId) continue;
    documentsById.set(documentId, {
      id: documentId,
      title: sanitizeText(relation.title || '未知来源', 160),
      content: '',
      currentVersionId: source.currentVersionId ?? null,
      revision: source.currentRevision ?? null,
      contentHash: source.currentContentHash ?? null
    });
  }
  ...
  const relationSources = (relations?.relatedDocuments || []).flatMap(relation => (relation.sourceRefs || []).map(source => ({
    ...
    excerpt: sanitizeText(source.excerpt || source.quote || source.snippet || '', MAX_EXCERPT),
    ...(source.evidenceSchemaVersion !== undefined ? { evidenceSchemaVersion: source.evidenceSchemaVersion } : {}),
    ...(source.evidenceId ? { evidenceId: source.evidenceId } : {}),
    ...(source.provenance ? { provenance: source.provenance } : {}),
    ...(source.excerptHash ? { excerptHash: source.excerptHash } : {}),
    contentVersionId: source.contentVersionId ?? source.sourceVersionId ?? null,
    revision: source.revision ?? source.sourceRevision ?? null,
    contentHash: source.contentHash ?? source.sourceContentHash ?? null,
    ...(source.currentVersionId != null ? { currentVersionId: source.currentVersionId } : {}),
    ...(source.currentRevision != null ? { currentRevision: source.currentRevision } : {}),
    ...(source.currentContentHash != null ? { currentContentHash: source.currentContentHash } : {}),
    ...
    ...(source.speaker ? { speaker: sanitizeText(source.speaker, 80) } : {})
  })));
...
export function createAnswerArtifactPayload(kind, { question = '', answer = '', citations = [], relations = {} } = {}) {
  if (!['note', 'task', 'writing', 'chart', 'problem'].includes(kind)) throw new TypeError('Unsupported answer artifact kind: ' + kind);
  ...
  if (kind === 'note') {
    ...
    const draft = problemNoteDraft({ question: safeQuestion });
    title = draft.title;
    content = appendWikiLinksToNote(applyAssistantAnswerToProblemNote({
