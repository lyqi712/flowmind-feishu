...
function itemFromRow(row) {
  ...
  return { id: row.id, sourceConnectionId: row.source_connection_id, spaceId: row.space_id,
    ...
    title: row.title, content: row.current_content, revision: row.revision, contentHash: row.content_hash,
...
export class ContentRepository {
  ...
  listContentItems(filters = {}) {
    ...
    const limit = Math.max(1, Math.min(Number(filters.limit) || 100, 1000)), offset = Math.max(0, Number(filters.offset) || 0);
    return this.db.prepare(`SELECT c.* FROM content_items c ${where.length ? `WHERE ${where.join(' AND ')}` : ''}
  ...
  migrateLegacyState(state, { sourcePath = '[object]', sourceHash } = {}) {
    ...
    return this.transaction(() => {
      const stats = { sourceConnections: 0, spaces: 0, documents: { created: 0, versioned: 0, unchanged: 0, restored: 0 },
        totalDocuments: Array.isArray(state.documents) ? state.documents.length : 0 };
