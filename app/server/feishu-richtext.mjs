...
function blockAsset(block = {}) {
  if (block.image?.token) return { kind: 'image', token: block.image.token, fileName: block.image.name || `feishu-image-${block.image.token.slice(-8)}.png`, mimeType: block.image.mime_type || 'image/png', width: block.image.width, height: block.image.height };
  if (block.file?.token) return { kind: 'file', token: block.file.token, fileName: block.file.name || `feishu-file-${block.file.token.slice(-8)}`, mimeType: block.file.mime_type || 'application/octet-stream' };
  return null;
}
