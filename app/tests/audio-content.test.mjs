...
async function harness(service = { transcribe: async () => transcript, close: async () => {} }) {
  ...
  const app = await createInitializedApp({ stateFile: join(root, 'state.json'), env: {}, ocrService: false, transcriptionService: service, modelService: createFakeModelService(), modelOptions: { secretFile: join(root, 'model.enc'), masterKeyFile: join(root, 'model.key') }, feishuOptions: { secretFile: join(root, 'feishu.enc'), masterKeyFile: join(root, 'feishu.key') } });
...
test('audio upload persists original and returns timestamp-aware chunks and citations', async () => {
  ...
  try {
    ...
    const chat = await fetch(`${h.base}/api/chat/stream`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ query: 'Rollback owner', documentIds: [itemId] }) }); const done = ndjson(await chat.text()).find(event => event.type === 'done'); assert.match(done.citations[0].anchor, /^time:2\.5-5:chars:\d+-\d+$/); assert.equal(done.citations[0].pageAnchor, 'time:2.5-5'); assert.equal(done.citations[0].timeStart, 2.5); assert.equal(done.citations[0].speaker, 'Bob');
