...
test('工作区恢复的知识图谱使用服务端索引快照，而不是兼容图谱作为主图', () => {
  ...
  assert.match(appSource, /async function requestGraphSnapshot\(\)/);
  assert.match(appSource, /function invalidateGraphData\(\)/);
  assert.match(appSource, /graph=\{graphData \|\| EMPTY_INDEXED_GRAPH\}/);
  assert.match(appSource, /initialRootId=\{graphFocus\?\.documentId \|\| ''\}/);
  assert.match(componentSource, /initialRootId = ''/);
  assert.match(componentSource, /export function resolveGraphNodeId/);
  const graph = graphModule.buildKnowledgeGraph({ documents, notes });
  assert.equal(graphModule.resolveGraphNodeId(graph, 'doc-alpha'), 'document:doc-alpha');
...
test('SSR 暴露完整搜索、过滤、局部图谱、箭头、缩放和关闭契约', () => {
  ...
  for (const fragment of [
    ...
    "event.key === 'Enter' || event.key === ' '", 'function activateNode(node)', 'function openSelectedNode(node = selectedNode)',
    'openKnowledgeGraphNode(selectedNode, { onOpenDocument, onOpenNote', 'GraphSuggestionList',
...
test('没有显式来源时，主图不会把共同主题推断为关系', () => {
  const graph = graphModule.buildKnowledgeGraph({ documents: [
    { id: 'doc-memory-a', title: '记忆系统设计', content: '记忆系统用于长期项目和知识库工作流。' },
    { id: 'doc-memory-b', title: '知识库工作流', content: '长期项目需要稳定的记忆系统和知识库工作流。' }
  ] });
  assert.equal(graph.edges.some(edge => edge.type === 'semantic'), false);
  assert.equal(graph.edges.some(edge => edge.type === 'link' || edge.type === 'source'), false);
});

test('Sigma waits for a measurable container before mounting', () => {
  assert.equal(graphModule.graphContainerHasSize({ getBoundingClientRect: () => ({ width: 0, height: 400 }) }), false);
  assert.equal(graphModule.graphContainerHasSize({ getBoundingClientRect: () => ({ width: 400, height: 0 }) }), false);
  assert.equal(graphModule.graphContainerHasSize({ getBoundingClientRect: () => ({ width: 400, height: 300 }) }), true);
  assert.ok(componentSource.includes('waitForSize'));
  assert.ok(componentSource.includes('requestAnimationFrame'));
  assert.ok(componentSource.includes('if (graphContainerHasSize(container)) renderer?.refresh()'));
});

test('Sigma 双击打开文档或笔记，而不是只选中或缩放', () => {
  assert.ok(componentSource.includes("renderer.on('clickNode'"));
  assert.ok(componentSource.includes("renderer.on('doubleClickNode'"));
  assert.ok(componentSource.includes('preventSigmaDefault?.()'));
  assert.ok(componentSource.includes('openRef.current?.(node)'));
  assert.ok(componentSource.includes('onOpen={nodeId =>'));
  assert.ok(componentSource.includes('if (node) openSelectedNode(node);'));
  assert.ok(componentSource.includes('onDoubleClick={() => openSelectedNode(node)}'));
  assert.ok(componentSource.includes('const structureKey = useMemo'));
  assert.ok(!componentSource.includes('const renderKey = useMemo'));
  assert.ok(componentSource.includes('function writeSigmaGraphHits'));
  assert.ok(componentSource.includes('function applySigmaSelection'));
  assert.ok(componentSource.includes("container.dataset.graphHits = JSON.stringify(hits)"));
  assert.ok(componentSource.includes("container.dataset.selectedNodeId = nodeId || ''"));
  assert.ok(componentSource.includes('renderer.framedGraphToViewport'));
  assert.ok(componentSource.includes("renderer.getCamera().on('updated'"));
  assert.ok(componentSource.includes('applySigmaSelection(rendererRef.current, containerRef.current, selectedNodeId)'));
  assert.match(componentSource, /}, \[structureKey\]\);/);
});
