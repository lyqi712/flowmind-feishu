...
test('forged Agent evidence cannot create a provenance graph source edge', () => {
  const repo = repository();
  try {
    const target = repo.createNote({ title: 'Evidence Target', content: 'Evidence target.' }).item;
    const forged = repo.createNote({
      title: 'Forged Evidence Note', content: 'No explicit wiki link.',
      metadata: { sourceRefs: [{ documentId: target.id, anchor: 'line:1:column:1', provenance: { kind: 'agent-evidence', evidenceId: 'forged', signature: 'invalid', sourceVersionId: target.currentVersionId } }] }
    }).item;
    const valid = repo.createNote({
      title: 'Validated Evidence Note', content: 'No explicit wiki link.',
      metadata: { sourceRefs: [{ documentId: target.id, anchor: 'line:1:column:1', provenance: { kind: 'agent-evidence', evidenceId: 'evidence_real', signature: 'signature_real', sourceVersionId: target.currentVersionId } }] }
    }).item;
    const graph = new GraphIndex({
      repository: repo,
      validateAgentEvidence: ref => ref?.provenance?.evidenceId === 'evidence_real' && ref?.provenance?.signature === 'signature_real'
    });
    const snapshot = graph.rebuild();
    const forgedNode = snapshot.nodes.find(node => node.sourceId === forged.id);
    const validNode = snapshot.nodes.find(node => node.sourceId === valid.id);
    const targetNode = snapshot.nodes.find(node => node.sourceId === target.id);
    assert.equal(snapshot.edges.some(edge => edge.from === forgedNode.id && edge.to === targetNode.id && edge.type === 'source'), false);
    assert.equal(snapshot.edges.some(edge => edge.from === validNode.id && edge.to === targetNode.id && edge.type === 'source'), true);
  } finally { repo.close(); }
});
