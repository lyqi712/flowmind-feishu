...
async function harness({ modelService, ocrService = false, chatAttachments } = {}) {
  ...
  const app = await createInitializedApp({
    stateFile, env: {}, modelService: modelService || createFakeModelService(), ocrService, transcriptionService: false,
...
test('chat accepts imported contentItemId/documentId and forces generic attachment questions into citations', async () => {
  ...
  try {
    ...
    const byDocument = await chat(h, { question: '文档中的 Owner、Deadline 和发布清单是什么？', documentIds: [item.id] });
...
test('inline data-url/base64 files reuse unified parsing and enter model context without persisting payloads', async () => {
  ...
  try {
    ...
    const list = await (await fetch(`${h.base}/api/conversations`)).json();
    const conversationId = (list.conversations || []).at(-1)?.id;
    assert.ok(conversationId);
    const saved = await (await fetch(`${h.base}/api/conversations/${conversationId}`)).json();
    const lastConversation = saved.conversation;
