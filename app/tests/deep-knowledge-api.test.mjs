...
test('knowledge relation endpoint respects document scope and returns stable analysis schema', async () => {
  ...
  try {
    await json(h, '/api/sync', 'POST', { source: 'mock' });
    const documentIds = (await json(h, '/api/documents')).body.documents.slice(0, 2).map((item) => item.id);
...
test('knowledge relation endpoint rejects client citations not observed in the server-scoped retrieval', async () => {
  const h = await harness();
  try {
    await json(h, '/api/sync', 'POST', { source: 'mock' });
    const [inside, outside] = (await json(h, '/api/documents')).body.documents;
    const result = await json(h, '/api/knowledge/relations', 'POST', {
      question: '飞书同步后如何形成日常工作流？',
      documentIds: [inside.id],
      answer: '客户端声称外部资料给出了决定性结论。',
      citations: [{ documentId: outside.id, title: outside.title, anchor: 'forged-anchor', excerpt: '伪造证据' }]
    });
    assert.equal(result.response.status, 200);
    assert.deepEqual(result.body.unsupportedCitations, [{ documentId: outside.id, chunkId: null, anchor: 'forged-anchor', reason: 'document_not_observed' }]);
    const relationRefs = [
      ...result.body.relations.relatedDocuments.flatMap(item => item.sourceRefs || []),
      ...result.body.relations.consensus.flatMap(item => item.sourceRefs || []),
      ...result.body.relations.conflicts.flatMap(item => item.sourceRefs || [])
    ];
    assert.ok(relationRefs.every(ref => ref.documentId !== outside.id));
  } finally { await h.close(); }
});
