...
async function harness(recognizeImpl = recognizeFixture) {
  ...
  const app = await createInitializedApp({ stateFile: join(root, 'state.json'), env: {}, modelService: createFakeModelService(), contentOptions: { ingestion: { parsers: { '.png': parser, '.jpg': parser, '.jpeg': parser, '.webp': parser } } }, modelOptions: { secretFile: join(root, 'model.enc'), masterKeyFile: join(root, 'model.key') }, feishuOptions: { secretFile: join(root, 'feishu.enc'), masterKeyFile: join(root, 'feishu.key') } });
...
test('image upload persists original, indexes region-aware chunks and returns region citation', async () => {
  ...
  try {
    ...
    assert.match(done.citations[0].anchor, /^page:1:region:1:chars:\d+-\d+$/); assert.equal(done.citations[0].pageAnchor, 'page:1:region:1'); assert.equal(done.citations[0].pageNumber, 1);
