...
test('all four settings sections are real controlled navigation actions', () => {
  includesAll(component, [
    "{ id: SECTION_APPEARANCE, label: '通用设置'",
  ...
  assert.doesNotMatch(sourceSection(component, 'export function SettingsSidebar', 'function AppearanceSettingsSection'), /useState\s*\(/, 'sidebar must not own the active section');
});

test('appearance section exposes density and display controls', () => {
  includesAll(component, [
    ...
    '界面密度',
    'onToggleCompact',
    'data-settings-panel={SECTION_APPEARANCE}'
  ], 'appearance settings');
});
...
test('knowledge section loads Feishu and content status and exposes refresh plus wizard actions', () => {
  includesAll(component, [
    ...
    "requestJson(fetcher, '/api/feishu/oauth/logout'" ,
