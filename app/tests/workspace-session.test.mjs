...
import {
  ...
  findChatTabByConversationId,
  isPersistedWorkspaceTab,
...
test('opening the same conversation reuses the existing chat tab instead of duplicating it', () => {
  let state = createInitialWorkspaceSession();
  state = reduce(state, {
    type: 'OPEN_TAB',
    tab: { id: 'chat-1786859607361', kind: 'chat', type: 'chat', route: 'knowledge', title: 'Hermes 和 Agent Loop 有什么区别？', chat: { conversationId: 'conv-hermes' } }
  });
  state = reduce(state, {
    type: 'OPEN_TAB',
    tab: { id: 'chat-1786862133111', kind: 'chat', type: 'chat', route: 'knowledge', title: 'Hermes 和 Agent Loop 有什么区别？', chat: { conversationId: 'conv-hermes' } }
  });
  assert.equal(state.tabs.length, 1);
  assert.equal(state.tabs[0].id, 'chat-1786859607361');
  assert.equal(state.activeTabId, 'chat-1786859607361');
  assert.equal(findChatTabByConversationId(state.tabs, 'conv-hermes')?.id, 'chat-1786859607361');

  state = reduce(state, {
    type: 'OPEN_TAB',
    tab: { id: 'module-knowledge', kind: 'chat', type: 'chat', route: 'knowledge', title: '知识库', chat: { conversationId: 'conv-hermes' } }
  });
  assert.equal(state.tabs.length, 2);
  assert.ok(state.tabs.some(tab => tab.id === 'module-knowledge'));
});
...
test('设置和 Copilot 等壳层标签刷新后不恢复，回到首页而不是摊在顶栏', () => {
  assert.equal(isPersistedWorkspaceTab({ id: 'module-settings', route: 'settings' }), false);
  assert.equal(isPersistedWorkspaceTab({ id: 'module-copilots', route: 'copilots' }), false);
  assert.equal(isPersistedWorkspaceTab({ id: 'module-notes', route: 'notes' }), true);
  assert.equal(isPersistedWorkspaceTab({ id: 'module-knowledge', route: 'knowledge', kind: 'chat' }), true);
  const restored = normalizeWorkspaceSession({
    tabs: [
      { id: 'module-notes', kind: 'module', route: 'notes', title: '笔记' },
      { id: 'module-settings', kind: 'module', route: 'settings', title: '设置' },
      { id: 'module-copilots', kind: 'module', route: 'copilots', title: 'Copilot' },
      { id: 'doc-1', kind: 'document', title: '方案', resourceId: 'c1' }
    ],
    activeTabId: 'module-settings'
  });
  assert.deepEqual(restored.tabs.map(tab => tab.id), ['module-notes', 'doc-1']);
