...
test('document title opens the reader while a separate control changes question scope', () => {
  assert.match(source, /className="doc-open"/);
  assert.match(source, /onClick=\{\(\) => onOpenDocument\?\.\(doc\)\}/);
...
test('reader source notes preserve sourceRefs and notes can return to the source', () => {
  ...
  assert.match(source, /<NotesModule onToast=\{notify\} onOpenDocument=\{openRelatedDocument\}/);
});
