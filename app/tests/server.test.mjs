...
async function createHarness(options = {}) {
  ...
  const app = createApp({ ...options, stateFile });
...
test('十一个 Skill 均输出流式步骤、持久化来源与真实任务产物', async () => {
  const harness = await createHarness({
    taskArtifactOptions: {
  ...
  try {
    ...
    const skillIds = ['summary', 'compare', 'research-report', 'mind-map', 'quiz', 'podcast', 'document-insight', 'smart-writing', 'action-items', 'faq', 'timeline'];
    ...
    const state = await fetch(`${harness.baseUrl}/api/state`).then((result) => result.json());
    assert.equal(state.skillRuns.length, 11);
