...
export function groundedAskKnowledge(documents, question, { limit = 5 } = {}) {
  ...
  return {
    ...
    citationStatus: 'local-evidence'
  };
}

async function askViaRunningApp(apiBaseUrl, { question, documentIds, limit }) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 8000);
  try {
    const response = await fetch(`${String(apiBaseUrl || '').replace(/\/$/, '')}/api/agent/run`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ question, query: question, mode: 'auto', documentIds, limit }),
    ...
    if (!done?.result?.answer) return null;
    if (done.result.citationStatus === 'empty_retrieval' || !(done.result.sourceRefs || []).length) {
      if (done.result.citationStatus === 'empty_retrieval') {
        return { question, answer: EMPTY_RETRIEVAL_ANSWER, citations: [], citationStatus: 'empty_retrieval' };
      }
      return null;
    }
    return {
      ...
      citations: done.result.sourceRefs,
      citationStatus: done.result.citationStatus || 'grounded-observation'
    };
  } catch {
    return null;
  } finally {
...
export async function createFlowMindMcpServer({
  ...
  server.registerTool('ask_knowledge', {
    ...
    const local = groundedAskKnowledge(documents, question, { limit });
    if (local.citationStatus === 'empty_retrieval') return jsonText(local);
    const live = await askViaRunningApp(apiBaseUrl, { question, documentIds, limit });
    return jsonText(live || local);
  });
