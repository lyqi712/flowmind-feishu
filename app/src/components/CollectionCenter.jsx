...
import { AlertCircle, ArrowRight, CheckCircle2, FileText, FolderOpen, Globe, Library, Link2, LoaderCircle, Sparkles, Upload, X } from 'lucide-react';
import './CollectionCenter.css';
import { normalizeClientBrowseUrl } from '../workspace/web-browse.js';
...
export function CollectionCenter({ open = true, onClose, onOpenFeishu, onImportFiles, onImportText, onOpenLibrary, onOpenWeb }) {
  const titleId = useId();
  ...
  function handleOpenWeb(event) {
    ...
    if (!raw || typeof onOpenWeb !== 'function') return;
    setError('');
    try {
      const url = normalizeClientBrowseUrl(raw).href;
      onOpenWeb(url);
    } catch (currentError) {
      setError(readableError(currentError, '网址无效'));
    }
  }
