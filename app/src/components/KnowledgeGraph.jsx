...
import { ArrowRight, CircleDot, FileText, Focus, FolderTree, Link2, LocateFixed, Maximize2, MessageSquarePlus, Network, NotebookPen, Search, Sparkles, Tag, X, ZoomIn, ZoomOut } from 'lucide-react';
...
const NODE_COLORS = Object.freeze({ document: '#7561dc', note: '#2f9e72', tag: '#d58a25', folder: '#267cb9', unresolved: '#9b98a4' });
const safeArray = value => Array.isArray(value) ? value : value == null ? [] : [value];
const cleanText = value => String(value ?? '').trim();
const GRAPH_STOP_WORDS = new Set(['的','了','和','与','及','是','在','对','中','个','这','那','一个','当前','相关','内容','文档','资料','方案','说明','知识库','知识图谱','系统','项目','工作流','用户','模型','使用','需要','可以','通过','进行','提供','支持','问题','回答','帮助','建立','实现','管理','处理','分析','输出','生成','全部','一些','各种','不同','这样','其中','这个','这些','作为','以及','同时','后续','已经','没有','更多','直接','根据','为了','对于','包括']);

function graphTerms(item) {
  const title = cleanText(item?.title);
  const tags = uniqueTags(item?.tags, item?.metadata?.tags).join(' ');
  const body = cleanText(item?.content).slice(0, 5000);
  const terms = new Map();
  const add = (value, weight = 1, origin = 'body') => {
    const key = cleanText(value).toLocaleLowerCase();
    if (!key || key.length < 3 || GRAPH_STOP_WORDS.has(key) || /^\d+$/u.test(key)) return;
    const current = terms.get(key) || { score: 0, named: false };
    terms.set(key, { score: current.score + weight, named: current.named || origin === 'title' || origin === 'tag' });
  };
  const addText = (value, weight, origin, minimumGram = 4) => {
    const text = cleanText(value);
    for (const match of text.matchAll(/[\u3400-\u9fff]{2,32}|[A-Za-z][A-Za-z0-9_-]{2,24}/g)) {
      const phrase = match[0];
      if (!/^[\u3400-\u9fff]+$/u.test(phrase)) { add(phrase, weight, origin); continue; }
      if (phrase.length <= 16) add(phrase, weight, origin);
      const maximumGram = Math.min(6, phrase.length);
      for (let size = minimumGram; size <= maximumGram; size += 1) {
        for (let index = 0; index <= phrase.length - size; index += 1) add(phrase.slice(index, index + size), weight * (size >= 5 ? 0.9 : 0.6), origin);
      }
    }
  };
  addText(title, 5, 'title');
  addText(tags, 8, 'tag');
  addText(body, 0.75, 'body');
  return terms;
}

function sharedGraphTerms(leftTerms, rightTerms, documentFrequency, entityCount) {
  const commonLimit = entityCount <= 4 ? entityCount + 1 : Math.min(4, Math.max(3, Math.ceil(entityCount * 0.18)));
  return [...leftTerms.keys()].filter(term => rightTerms.has(term) && (documentFrequency.get(term) || 0) < commonLimit)
    .map(term => {
      const left = leftTerms.get(term), right = rightTerms.get(term);
      return { term, score: left.score + right.score, namedInBoth: left.named && right.named };
    })
...
export function buildKnowledgeGraph({ documents = [], notes = [] } = {}) {
  ...
  const entityRows = [...documentRows.map((item, index) => ({ item, nodeId: documentNodeIds[index] })), ...noteRows.map((item, index) => ({ item, nodeId: noteNodeIds[index] }))];
  const termByNodeId = new Map(entityRows.map(row => [row.nodeId, graphTerms(row.item)]));
  const documentFrequency = new Map();
  for (const terms of termByNodeId.values()) for (const term of terms.keys()) documentFrequency.set(term, (documentFrequency.get(term) || 0) + 1);
  for (let leftIndex = 0; leftIndex < entityRows.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < entityRows.length; rightIndex += 1) {
      const left = entityRows[leftIndex], right = entityRows[rightIndex];
      const shared = sharedGraphTerms(termByNodeId.get(left.nodeId) || new Map(), termByNodeId.get(right.nodeId) || new Map(), documentFrequency, entityRows.length);
      const exactNamed = shared.filter(entry => entry.namedInBoth && entry.score >= 8);
      const strongTerms = shared.filter(entry => entry.score >= 2.2);
      if (!exactNamed.length && strongTerms.length < 2) continue;
      const terms = shared.slice(0, 3).map(entry => entry.term);
      addEdge({ from: left.nodeId, to: right.nodeId, type: 'semantic', directed: false, resolved: true, weight: Math.max(1, shared.length), label: '共同主题：' + terms.join('、') });
    }
  }
  if (entityRows.length > 4) {
    const semanticEntries = [...edges.entries()].filter(([, edge]) => edge.type === 'semantic').sort((left, right) => right[1].weight - left[1].weight || right[1].label.length - left[1].label.length);
    const semanticDegree = new Map();
    const keep = new Set();
    const maxSemanticEdges = Math.max(1, Math.ceil(entityRows.length * 0.9));
    for (const [key, edge] of semanticEntries) {
      if (keep.size >= maxSemanticEdges) break;
      if ((semanticDegree.get(edge.from) || 0) >= 2 || (semanticDegree.get(edge.to) || 0) >= 2) continue;
      keep.add(key);
      semanticDegree.set(edge.from, (semanticDegree.get(edge.from) || 0) + 1);
      semanticDegree.set(edge.to, (semanticDegree.get(edge.to) || 0) + 1);
    }
    for (const [key] of semanticEntries) if (!keep.has(key)) edges.delete(key);
  }
...
export function openKnowledgeGraphNode(node, { onOpenDocument, onOpenNote } = {}) {
  if (!node) return false;
  if (node.type === 'document') { onOpenDocument?.(node.raw || { id: node.sourceId, title: node.label }); return typeof onOpenDocument === 'function'; }
  if (node.type === 'note') { onOpenNote?.(node.raw || { id: node.sourceId, title: node.label }); return typeof onOpenNote === 'function'; }
  return false;
}
...
function appendSuggestionLayer(graph, enabled) {
  if (!enabled || !graph?.suggestions?.length) return graph;
  const nodeIds = new Set(graph.nodes.map(node => node.id));
  const suggestionEdges = graph.suggestions
    .filter(suggestion => suggestion.status === 'pending' && nodeIds.has(suggestion.sourceNodeId) && nodeIds.has(suggestion.targetNodeId))
    .map(suggestion => ({
      id: `suggestion:${suggestion.id}`,
      from: suggestion.sourceNodeId,
      to: suggestion.targetNodeId,
      type: 'suggestion',
      directed: true,
      resolved: true,
      weight: 1,
      label: suggestion.reason || 'AI link proposal',
      suggestionId: suggestion.id,
      provenance: { kind: 'ai-suggestion', evidence: suggestion.evidence || [] }
    }));
  return { ...graph, edges: [...graph.edges, ...suggestionEdges] };
}
...
export function KnowledgeGraph({ documents = [], notes = [], graph: graphData = null, loading = false, onOpenDocument, onOpenNote, onAskNode, onCreateNote, onConfirmSuggestion, onRefreshGraph, onClose }) {
  ...
  const [localRootId, setLocalRootId] = useState('');
  ...
  const layeredGraph = useMemo(() => appendSuggestionLayer(fullGraph, showSuggestions), [fullGraph, showSuggestions]);
  const effectiveRootId = localRootId || selectedNodeId || fullGraph.nodes.find(node => node.type === 'document' || node.type === 'note')?.id || '';
  const scopedGraph = useMemo(() => localMode ? buildLocalGraph(layeredGraph, effectiveRootId, localJumps) : layeredGraph, [layeredGraph, localMode, effectiveRootId, localJumps]);
  ...
  const selectedSuggestions = selectedNode ? (fullGraph.suggestions || []).filter(suggestion => suggestion.status === 'pending' && (suggestion.sourceNodeId === selectedNode.id || suggestion.targetNodeId === selectedNode.id)) : [];
  ...
  function activateNode(node) {
    ...
    openKnowledgeGraphNode(node, { onOpenDocument, onOpenNote });
  }
  ...
  function selectRelated(node) { setSelectedNodeId(node.id); if (localMode) setLocalRootId(node.id); }
    ...
    <section className="knowledge-graph" aria-label="知识观察">
      <header className="knowledge-graph-header">
        <div className="knowledge-graph-heading">
          ...
          <div><h2>知识观察</h2><p>{fullGraph.nodes.length} 个节点 · {fullGraph.edges.length} 条关系{unresolvedCount ? ` · ${unresolvedCount} 条待处理链接` : ''}</p><small className="knowledge-graph-subtitle">仅显示可回查的显式链接、来源、标签和文件夹关系</small></div>
        </div>
      ...
      <div className="knowledge-graph-toolbar">
        ...
        <div className="knowledge-graph-filters" aria-label="图谱过滤器">
          ...
          <button type="button" aria-pressed={showSuggestions} className={showSuggestions ? 'is-active suggestion' : ''} disabled={!fullGraph.suggestions?.length} onClick={() => setShowSuggestions(current => !current)}><Sparkles size={13}/>AI 建议 <span>{fullGraph.suggestions?.length || 0}</span></button>
        </div>
      ...
      <div className={'knowledge-graph-workspace' + (selectedNode || fullGraph.nodes.length ? ' has-inspector' : '')}>
        ...
        {!selectedNode && fullGraph.nodes.length > 0 && <GraphRelationOverview relations={relationSummary} onSelect={nodeId => setSelectedNodeId(nodeId)}/>}
...
<div className="knowledge-graph-inspector-stats"><span><b>{selectedNode.degree}</b>关系</span><span><b>{relations.incoming.length}</b>反向</span><span><b>{relations.outgoing.length}</b>出向</span></div>
          ...
          {selectedSuggestions.length > 0 && <section className="knowledge-graph-suggestions"><h4><Sparkles size={14}/>待确认建议</h4>{selectedSuggestions.map(suggestion => <div key={suggestion.id}><p>{suggestion.reason || 'AI 根据可回查证据建议建立链接。'}</p><button type="button" onClick={() => onConfirmSuggestion?.(suggestion)}>确认建议</button></div>)}</section>}
...
function SigmaGraphCanvas({ graph, positions, selectedNodeId, showArrows, onActivate }) {
  const containerRef = useRef(null);
  const activateRef = useRef(onActivate);
  activateRef.current = onActivate;
  const renderKey = useMemo(() => [
    ...(graph.nodes || []).map(node => `${node.id}:${positions[node.id]?.x || 0}:${positions[node.id]?.y || 0}:${node.id === selectedNodeId ? 'selected' : ''}`),
    ...(graph.edges || []).map(edge => `${edge.id}:${edge.from}:${edge.to}:${edge.type}:${showArrows ? 'arrows' : 'plain'}`)
  ].join('|'), [graph, positions, selectedNodeId, showArrows]);
      ...
      try {
        ...
        for (const edge of graph.edges) {
          ...
          const attributes = {
            ...
            color: edge.type === 'source' ? '#4d9b78' : edge.type === 'tag' ? '#cb913e' : edge.type === 'suggestion' ? '#c66eaa' : '#a6a0b2',
            label: edge.label || edgeLabel(edge.type),
...
function GraphRelationOverview({ relations, onSelect }) {
