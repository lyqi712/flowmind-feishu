...
export function writingSaveSnapshot(draft = {}) {
  return {
    ...
    contentVersionId: draft?.contentVersionId ?? draft?.currentVersionId ?? null,
    baseVersion: draft?.baseVersion ?? draft?.contentVersionId ?? draft?.currentVersionId ?? null
  };
}

async function sendWritingSnapshot(snapshot, { keepalive } = {}) {
  const data = await sendJsonDocument(`/api/writing/drafts/${encodeURIComponent(snapshot.id)}`, jsonBodyWithBaseVersion({
  ...
  return { draft: requireSavedRecord(data, 'draft', '草稿保存响应缺少正文，已保留未保存编辑') };
}
...
export function WritingModule({ onToast, initialDraftId = '', onOpenDocument }) {
  ...
  useLayoutEffect(() => {
    ...
    return () => aiRequestRef.current.invalidate();
  }, [draft?.id]);
  ...
  async function load() {
    ...
    if (current?.id && (dirtyRef.current || saveControllerRef.current.hasUnsaved(current.id))) {
      saveControllerRef.current.schedule(writingSaveSnapshot(current));
  ...
  useEffect(() => {
    ...
    const unregisterGuard = registerWorkspaceSaveGuard(async () => {
      ...
      if (current?.id && (dirtyRef.current || controller.hasUnsaved(current.id) || controller.inspect(current.id)?.hasTimer)) {
        controller.schedule(writingSaveSnapshot(current));
      }
  ...
  async function select(item) {
    ...
    closeAiWriting();
    saveControllerRef.current.acceptBaseline(item.id, item);
  ...
  function update(patch, aiSnapshot = null) {
    ...
    setDraft(current => {
      ...
      if (aiSnapshot && (!aiRequestRef.current.isCurrent(aiSnapshot.requestToken, current?.id)
  ...
  async function createDraft() {
    ...
    try {
      ...
      await select(data.draft);
      onToast?.('已创建写作草稿');
    } catch (error) { onToast?.(error.message, 'error'); }
    finally { setBusy(''); }
  }

  function handleWritingSaveChange(event) {
    ...
    if (event.status === 'saved' && !event.conflict) {
      const saved = event.result?.draft;
      if (saved) {
        setDrafts(list => list.map(item => item.id === saved.id ? saved : item));
    ...
    if (event.status === 'error' && event.id === currentId) {
      setSaveError(event.error || '草稿保存失败');
      setBusy(busy => busy === 'save' ? '' : busy);
      onToast?.(event.error || '草稿保存失败', 'error');
    }
  }
  saveRuntimeRef.current = { send: sendWritingSnapshot, onChange: handleWritingSaveChange };

  async function save(silent = false) {
    if (!draft?.id) return;
    if (dirty) saveControllerRef.current.schedule(writingSaveSnapshot(draft));
    const result = saveError
      ? await saveControllerRef.current.retry(draft.id)
      : await saveControllerRef.current.flush(draft.id);
    if (result?.ok && !result.skipped && !silent) onToast?.('草稿已保存并生成版本');
  }
  ...
  async function runAiWriting(action) {
    ...
    try {
      ...
      const generated = await readWritingAiStream(response, { onDelta: result => publishAiWriting(requestToken, { result }) });
      ...
      publishAiWriting(requestToken, { status: 'error', error: error.message || 'AI 写作失败' });
    }
  }
  ...
  function applyAiWriting(mode) {
    ...
    try {
      const applied = applyWritingAiResult({ content: draft.content, result: aiWriter.result, range: aiWriter.range, mode });
      const result = update({ content: applied.content, sourceRefs: mergeWritingSourceRefs(draft.sourceRefs, aiWriter.citations) }, aiWriter);
