...
export function TranslationWorkbench({ translation, translations = [], sourceLanguage, targetLanguage, provider, glossary, busy, dirty, activeAnchor, onChangeSettings, onSelect, onGenerate, onSave, onClose, onLocate, onUpdateSegment, onExport, onToast }) {
  ...
  return <section className="translation-workbench">
    <header className="translation-head">
      <div><span><Languages size={17}/></span><div><b>对照翻译</b><small>段落与原文 page / region / time 引用保持联动</small></div></div>
      <button type="button" className="translation-close" onClick={onClose} title="返回原文"><X size={16}/></button>
    </header>
    ...
    {segments.length ? <div className="translation-table">
      ...
      {segments.map((row, index) => <article key={`${row.anchor || index}-${index}`} className={sameAnchor(activeAnchor, row.anchor) ? 'active' : ''}>
        <button type="button" className="translation-source" onClick={() => onLocate?.(row.anchor, row.pageNumber)}><small>{row.anchor || `片段 ${index + 1}`}{row.speaker ? ` · ${row.speaker}` : ''}</small><p>{row.sourceText}</p></button>
