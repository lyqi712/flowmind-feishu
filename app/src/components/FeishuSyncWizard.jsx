...
function normalizeSettings(data = {}) {
  ...
  return {
    ...
    appIdMasked: source.appIdMasked || '', documentUrls: Array.isArray(source.documentUrls) ? source.documentUrls : [],
  ...
  return <div className="feishu-wizard-backdrop" onMouseDown={() => canClose && onClose?.()}>
    <section className="feishu-wizard" onMouseDown={event => event.stopPropagation()}>
      <header className="fw-header">
        ...
        <button className="fw-close" disabled={!canClose} onClick={onClose}><X size={20}/></button>
      </header>
      <div className="fw-layout">
        ...
        <main className="fw-main">
          ...
          {busy === 'loading' ? <WizardLoading/> : <>
            {step === 0 && <CredentialsStep form={form} setForm={setForm} settings={settings} busy={busy} error={error} showSecret={showSecret} setShowSecret={setShowSecret} save={saveCredentials}/>}
...
function CredentialsStep({ form, setForm, settings, busy, error, showSecret, setShowSecret, save }) {
