...
function coverageDetails(message) {
  ...
  if (coverage && typeof coverage === 'object') {
    const covered = Number(coverage.covered ?? coverage.cited ?? coverage.supportedClaims ?? 0);
    const total = Number(coverage.total ?? coverage.claims ?? coverage.totalClaims ?? 0);
    ...
    return {
      ...
      uncoveredClaims: asArray(coverage.uncoveredClaims),
      unsupportedClaims: Number(coverage.unsupportedClaims ?? 0),
      level: coverage.level || ''
    };
  }
  const percent = clampScore(coverage) ?? 0;
  return { percent, detail: `${Math.round(percent)}% 已覆盖`, uncoveredClaims: [], unsupportedClaims: 0, level: '' };
}
...
function titleForSuggestion(item, side, relatedDocuments = []) {
  const contentId = side === 'source'
    ? item?.proposedPatch?.sourceContentItemId || item?.sourceContentItemId
    : item?.proposedPatch?.targetContentItemId || item?.targetContentItemId;
  const related = relatedDocuments.find(document => String(document.id || document.documentId) === String(contentId || ''));
  if (related?.title) return related.title;
  if (side === 'source') return item?.sourceTitle || item?.sourceNodeId || '来源';
  return item?.targetTitle || item?.targetNodeId || '目标';
}
...
function CoverageCard({ coverage, integrity, uncoveredClaims = [] }) {
  const status = integrity?.status;
  const notice = status === 'downgraded'
    ? '部分引用编号无法对应到来源，已降级为未覆盖结论，没有作废整段回答。'
    : status === 'empty'
      ? '知识库没有可引用证据，因此没有给出事实结论。'
      : '';
  return <article className="deep-answer-coverage-card">
    ...
    {notice ? <p className="deep-answer-coverage-notice" data-integrity={status}>{notice}</p> : null}
    {uncoveredClaims.length > 0 ? <div className="deep-answer-uncovered">
      <span>未被引用覆盖的结论</span>
      <ul aria-label="未被引用覆盖的结论">
        {uncoveredClaims.map((claim, index) => <li key={`uncovered-${index}`}>{displayText(claim)}</li>)}
      </ul>
    </div> : null}
  </article>;
}

export function DeepAnswerPanel({ message = {}, busy = false, onFollowUp, onOpenDocument, onCreateArtifact, onConfirmSuggestion }) {
  ...
  const uncoveredClaims = asArray(coverage?.uncoveredClaims);
  const citationIntegrity = message.citationIntegrity || message.integrity || message.analysis?.citationIntegrity;
  ...
  const graphSuggestions = asArray(message.graphSuggestions);
  ...
  const extrasSummary = (citations.length || relatedDocuments.length)
    ? `${citations.length || relatedDocuments.length} 篇资料`
    : '查看依据';
  ...
  const graphSuggestionSection = graphSuggestions.length > 0 ? <section className="deep-answer-block deep-answer-graph-suggestions" aria-label="待确认关系">
    <SectionTitle icon={Network} title="待确认关系" meta={`${graphSuggestions.length} 条`}/>
    <div>
      {graphSuggestions.map(item => {
        const sourceTitle = item.sourceTitle || item.fromTitle || titleForSuggestion(item, 'source', relatedDocuments);
        const targetTitle = item.targetTitle || item.toTitle || titleForSuggestion(item, 'target', relatedDocuments);
        const pending = !item.status || item.status === 'pending';
        return <div key={item.id} className="deep-answer-graph-suggestion">
          <span>{sourceTitle}</span>
          <GitCompareArrows size={13}/>
          <span>{targetTitle}</span>
          {item.reason ? <small>{item.reason}</small> : null}
          {pending && onConfirmSuggestion ? <div className="deep-answer-graph-suggestion-actions">
            <button type="button" disabled={isBusy} onClick={() => onConfirmSuggestion(item.id, true, message)}>确认写入图谱</button>
            <button type="button" disabled={isBusy} onClick={() => onConfirmSuggestion(item.id, false, message)}>忽略</button>
          </div> : item.status ? <small>{item.status === 'approved' ? '已写入图谱' : item.status === 'rejected' ? '已忽略' : item.status}</small> : null}
        </div>;
      })}
    </div>
  </section> : null;
  ...
  return <section className="deep-answer-panel" aria-label="深度回答分析" aria-busy={isBusy}>
    <details className="deep-answer-extras">
      <summary className="deep-answer-header">
      <div className="deep-answer-heading">
        <span className="deep-answer-heading-icon" aria-hidden="true"><Sparkles size={14}/></span>
        <div>
          <h2>回答依据</h2>
          <p>{extrasSummary}</p>
        </div>
      ...
      <div className="deep-answer-extras-body">
      {citations.length > 0 ? <div className="citations" aria-label="出处"><span>出处</span>{citations.map((citation, index) => <button type="button" key={citation.id || citation.documentId || index} onClick={() => onOpenDocument?.(citation, message)} title={citation.snippet || citation.excerpt || ''}><b>[{index + 1}]</b><span>{displayText(citation, `资料 ${index + 1}`)}</span></button>)}</div> : null}
      {graphSuggestionSection}
      <div className="deep-answer-artifacts" role="group" aria-label="将回答转为工作产物">
        <button type="button" disabled={isBusy} aria-label="把这次容易忘的点记下来" onClick={() => createArtifact('problem')}>
          <Lightbulb size={15}/><span>记这个问题</span>
        </button>
    ...
    {message.chartArtifact?.chartSpec ? <section className="deep-answer-block deep-answer-chart" aria-label={'\u8bc1\u636e\u56fe\u8868'}>
      ...
      <div className="deep-answer-chart-sources"><span>{'\u751f\u6210\u56fe\u8868'}</span>{(message.chartArtifact.sourceRefs || message.chartArtifact.chartSpec.sourceRefs || []).slice(0, 8).map((source, index) => <button type="button" key={(source.documentId || 'source') + index} onClick={() => onOpenDocument?.(source, message)}><span>[{index + 1}]</span>{source.title || source.documentId || '\u6765\u6e90\u6587\u6863'}<EvidenceStatusBadge evidence={source} compact />{source.anchor ? <small>{source.anchor}</small> : null}</button>)}</div>
    </section> : null}

    {followUps.length > 0 ? <section className="deep-answer-block deep-answer-follow-ups">
      <SectionTitle icon={Lightbulb} title="继续追问"/>
      <div>
    ...
    {showProcessDetails || topics.length > 0 || entities.length > 0 || bidirectionalLinks.length > 0 || relatedDocuments.length > 0 || hasSynthesis ? <details className="deep-answer-process">
      <summary className="deep-answer-process-summary">分析过程</summary>
      <div className="deep-answer-process-body">
        {showProcessDetails ? <div className="deep-answer-overview">
          ...
          {coverage ? <CoverageCard coverage={coverage} integrity={citationIntegrity} uncoveredClaims={uncoveredClaims}/> : null}
        </div> : null}
        ...
        {bidirectionalLinks.length > 0 ? <section className="deep-answer-block deep-answer-map-block">
          ...
          <div className="deep-answer-map-flow">
            ...
            <div className="deep-answer-bidirectional"><strong>双向关联</strong>{bidirectionalLinks.slice(0, 8).map((link, index) => <div key={`link-${index}`}><span>{link.fromTitle || link.fromDocumentId}</span><GitCompareArrows size={13}/><span>{link.toTitle || link.toDocumentId}</span><small>{link.reason || '共享知识线索'}</small></div>)}</div>
          </div>
        </section> : null}
