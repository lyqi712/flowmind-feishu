...
function workIdentity(item) {
  ...
  if (explicitId) return `id:${explicitId}`;
  const resource = nonEmptyString(item?.resourceId ?? item?.documentId ?? item?.noteId ?? item?.url);
...
function normalizeTask(task, index = 0) {
  ...
  const status = TASK_STATUSES.has(task.status) ? task.status : 'queued';
  return {
    ...task,
    id,
    status,
...
function normalizeTasks(tasks) {
  ...
  for (const [index, candidate] of asArray(tasks).entries()) {
    const task = normalizeTask(candidate, index);
    if (!task) continue;
...
export function normalizeWorkspaceSession(value, options = {}) {
  ...
  let tabs = enforceTabLimit(dedupeTabs(migrated.tabs), maxTabs, nonEmptyString(migrated.activeTabId));
  let activeTabId = nonEmptyString(migrated.activeTabId);
  if (!tabs.some((tab) => tab.id === activeTabId)) activeTabId = tabs.at(-1)?.id ?? null;
...
function openTab(state, action, options) {
  const candidate = normalizeTab(action.tab ?? action.payload, state.tabs.length);
  ...
  const merged = existing
    ? state.tabs.map((tab) => tab.id === existing.id ? { ...tab, ...candidate, id: existing.id } : tab)
    ...
    case 'ADD_RECENT_WORK': {
      ...
      return {
        ...
        recentWork: normalizeRecentWork(
          [...state.recentWork.filter((entry) => workIdentity(entry) !== workIdentity(item)), item],
