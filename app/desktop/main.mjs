...
if (!hasSingleInstanceLock) {
  ...
  desktopHost = await startDesktopHost({
    ...
    feishuOptions: { secretFile: path.join(userData, 'feishu-secret.enc'), masterKeyFile: path.join(userData, '.feishu-master-key') },
    contentOptions: { databasePath: path.join(userData, 'content.sqlite') },
    modelOptions: { secretFile: path.join(userData, 'model-secret.enc'), masterKeyFile: path.join(userData, '.model-master-key') },
    logger
  });
